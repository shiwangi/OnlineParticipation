...
public:
	int getMin(string road) {
	  ...
	  int i;
	  int l=road.length(),a,b,j,k,n=0;
    	    vector<pair<int,int> >store;
    	    queue<pair<int,char> >mark;
    	    mark.push(pair<int,char>(0,'R'));
    	    char exps;
    	    while(!mark.empty())
    	    {
    	        if(((mark.front()).second)=='R')
                    exps='G';
                else if(((mark.front()).second)=='G')
                    exps='B';
                else
                    exps='R';
                    
    	        for(i=((mark.front()).first)+1;i<l;i++)
    	        {
    	            if(road[i]==exps)
    	            {
                        mark.push(pair<int,char>(i,exps));
                        store.push_back(pair<int,int>(mark.front().first,i));
                        n++;
    	            }
    	        }
    	        mark.pop();
