...
public class XorTravelingSalesman {
    ...
    public int maxProfit(int[] cityValues, String[] roads) {
        ...
        for (int i = 0; i < l; i++) {
            city[i] = cityValues[i];
            for (int j = 0; j < l; j++) {
                a[i][j] = (roads[i].charAt(j) == 'Y') ? 1 : 0;
            }
        }
        visited = new boolean[l][1024];
        result = 0;
        solve(0, cityValues[0]);
        return result;
    }

    private void solve(int node, int curr_profit) {
        //System.out.println(node+ " "+curr_profit);
        Stack<P> s=new Stack<P>();
        s.push(new P(node,curr_profit));
        while(!s.isEmpty()){
           P jj= s.pop();
        result = Math.max(jj.profit, result);
        visited[jj.node][jj.profit] = true;
        for (int j = 0; j < l; j++) {
            p = jj.profit ^ city[j];
            if (a[jj.node][j] == 1 && visited[j][p]==false) {
                s.push(new P(j, p));
            }
        }
        }
